...
#define SIM_DEC_OP_GSVA_HOME_V1     0x10
...
static MemTxResult ubc_sim_dec_remote_io(BusControllerDev *ubc_dev,
    ...
    uint32_t len, bool strict, bool write, const GsvaHomeIdentity *identity);
...
#define UBC_GSVA_IO_CHUNK_MAX \
    (UBC_SIM_DEC_MAX_MSG_PAYLOAD - (uint32_t)MAX(sizeof(UBCGsvaIoV2Req), \
                            ...
                            void *dst, uint64_t length)
{
    ...
    if (resolved.strict_gsva) {
        ret = ubc_sim_dec_remote_io(ubc_dev, resolved.remote_uba,
                                    ...
                                    true, false,
                                    &(GsvaHomeIdentity){.key = resolved.gsva.key,
                                        .home_cna = resolved.gsva.home_cna,
                                        .token_id = resolved.gsva.token_id,
                                        .token_value = resolved.gsva.token_value,
                                        .backing_token_id = resolved.gsva.backing_token_id});
    } else {
...
static ObmmExportEntry *obmm_export_lookup(uint64_t uba, uint64_t len,
                                           uint32_t token_id);
static bool obmm_export_range_overlaps(uint64_t uba, uint64_t len);
                                 ...
                                 const uint8_t *data, uint32_t data_len)
{
    ...
    if (gsva_home_overlaps(&ubc_dev->gsva_home, hdr->remote_uba, data_len)) {
        qemu_log("GSVA_HOME: legacy write denied\n");
        return;
    }

    eff_tid = ubc_tid_or_auto(hdr->token_id);
    export_entry = obmm_export_lookup(hdr->remote_uba, data_len,
                                      hdr->token_id);
    if (export_entry) {
        ret = obmm_export_write_exact(ubc_dev, export_entry,
                                      hdr->remote_uba, data, data_len);
    } else if (obmm_export_range_overlaps(hdr->remote_uba, data_len)) {
        qemu_log("SIM_DEC_OBMM_EXPORT_IDENTITY_REJECT op=write uba=%#" PRIx64
                 " len=%u token=%u\n",
                 (uint64_t)hdr->remote_uba, data_len, hdr->token_id);
        ret = MEMTX_DECODE_ERROR;
    } else {
        ret = ubc_dma_write_local_data_tid_strict(
            ubc_dev, hdr->remote_uba, data, data_len, eff_tid);
    }
    if (ret != MEMTX_OK && !export_entry &&
        !obmm_export_range_overlaps(hdr->remote_uba, data_len)) {
        gsva_tables_init();
                       ...
                       uint32_t payload_len, uint32_t peer_cna)
{
    ...
    header_size = io->version == 2 ? sizeof(UBCGsvaIoV2Req) : sizeof(*io);
    if (!len || len > UBC_SIM_DEC_MAX_MSG_PAYLOAD -
        MAX(header_size, sizeof(UBCSimDecReadRespPldHdr)) ||
        len > UINT64_MAX - io->request.remote_uba ||
        payload_len != header_size + (io->write ? len : 0)) {
        return;
    }
    ...
    if (io->version == 2) {
        const UBCGsvaIoV2Req *v2 = (const UBCGsvaIoV2Req *)payload;

        if (v2->identity.backing_token_id != io->request.token_id ||
            gsva_home_acquire(&ubc_dev->gsva_home, ubc_dev->parent.cna,
                &v2->identity, io->request.remote_uba, len, io->write,
                &pin) != GSVA_OK) {
            goto reply;
        }
                                    ...
                                    uint32_t dcna)
{
    ...
    if (gsva_home_overlaps(&ubc_dev->gsva_home, req->remote_uba, req->read_len)) {
        UBCSimDecReadRespPldHdr denied = {
            .req_id = req->req_id,
            .status = UBC_SIM_DEC_READ_STATUS_ERROR,
        };

        ubc_send_msg_over_link(ubc_dev, link, dcna,
                              UBC_MSG_SUB_SIM_DEC_READ_RESP, &denied, sizeof(denied));
        return;
    }
    if (void_eligible &&
    ...
    export_entry = obmm_export_lookup(req->remote_uba, req->read_len,
                                      req->token_id);
    if (export_entry) {
        ret = obmm_export_read_exact(ubc_dev, export_entry,
                                     req->remote_uba,
                                     payload + sizeof(*resp),
                                     req->read_len);
        if (void_eligible) {
            qemu_log("SIM_DEC_OBMM_EXPORT_IDENTITY_ROUTE req=%u token=%u "
                     "export=%" PRIu64 " generation=%" PRIu64
                     " backing=%#" PRIx64 "\n",
                     req->req_id, req->token_id,
                     export_entry->export_mem_id, export_entry->generation,
                     export_entry->backing_uba);
        }
    } else if (obmm_export_range_overlaps(req->remote_uba,
                                          req->read_len)) {
        qemu_log("SIM_DEC_OBMM_EXPORT_IDENTITY_REJECT op=read req=%u "
                 "uba=%#" PRIx64 " len=%u token=%u\n",
                 req->req_id, (uint64_t)req->remote_uba,
                 req->read_len, req->token_id);
        ret = MEMTX_DECODE_ERROR;
    } else {
        ret = ubc_dma_read_local_data_tid_strict(
            ubc_dev, req->remote_uba, payload + sizeof(*resp),
            req->read_len, eff_tid);
    }
    if (ret != MEMTX_OK && !export_entry &&
        !obmm_export_range_overlaps(req->remote_uba, req->read_len)) {
        /*
    ...
    uint32_t len, bool strict, bool write, const GsvaHomeIdentity *identity)
{
    ...
    if (!strict) {
        return ubc_sim_dec_remote_io_body(ubc_dev, remote_uba, token_id,
                                          dcna, data, len, strict, write, identity);
    }
    ...
    uint32_t len, bool strict, bool write, const GsvaHomeIdentity *identity)
{
    ...
    if (!ubc_dev || !data || !len || dcna == 0 || len > UINT64_MAX - remote_uba ||
        (strict && (!ubc_dev->ummu || !token_id || token_id == UINT32_MAX ||
                    !identity || identity->home_cna != dcna ||
                    identity->backing_token_id != token_id)) ||
        (write && !strict)) {
        return MEMTX_DECODE_ERROR;
    }
    if (dcna == ubc_dev->parent.cna) {
        ...
        if (strict) {
            if (gsva_home_acquire(&ubc_dev->gsva_home, dcna, identity,
...
retry_chunk:
        ...
        if (strict) {
            uint32_t payload_len = sizeof(UBCGsvaIoV2Req) + (write ? chunk : 0);
            UBCGsvaIoV2Req *io = g_malloc0(payload_len);
            io->io.magic = UBC_GSVA_IO_MAGIC;
            io->io.version = 2;
            io->io.write = write;
            io->io.request = req;
...
static void ub_bus_controller_dev_finalize(Object *object)
{
    ...
    linqu_ub_gm_registry_free(ubc_dev->linqu_uapi_ub_gm_registry);
    ubc_dev->linqu_uapi_ub_gm_registry = NULL;
    gsva_home_destroy(&ubc_dev->gsva_home);
    if (ubc_dev->linqu_uapi_bridge) {
        linqu_ub_bridge_free(ubc_dev->linqu_uapi_bridge);
        ubc_dev->linqu_uapi_bridge = NULL;
    }
...
static bool obmm_export_range_overlaps(uint64_t uba, uint64_t len)
{
    ...
    QTAILQ_FOREACH(entry, &g_obmm_exports, next) {
        if (uba >= entry->remote_uba && len <= entry->size &&
            uba - entry->remote_uba <= entry->size - len) {
            return true;
        }
    }
...
static bool gsva_tlb_stable_matches(uint64_t va, uint64_t epoch,
                                    uint64_t segment_id)
{
    unsigned idx = gsva_tlb_stable_index(va);
    const GsvaTlbSideEntry *entry = &g_gsva_tlb_stable[idx];

    return entry->valid && entry->va == va && entry->epoch == epoch &&
                                ...
                                uint64_t *page_size)
{
    ...
    *page_size = SIM_DEC_PAGE_SIZE;
    ...
    /* TLB stale detection */
    int stale_rc = gsva_tlb_stale_check(page_va, coh_obj->epoch);
    if (stale_rc == GSVA_ERR_TLB_STALE) {
        qemu_log("GSVA_TLB: STALE va=%#" PRIx64 " tlb_epoch=%" PRIu64
                 " current_epoch=%" PRIu64 " segment_id=%#" PRIx64 "\n",
                 page_va,
                 g_gsva_tlb_stable[gsva_tlb_stable_index(page_va)].epoch,
                 coh_obj->epoch, coh_obj->key.segment_id);
        return GSVA_ERR_TLB_STALE;
    }

    /* Update TLB side table */
    tlb_hit = gsva_tlb_stable_matches(page_va, coh_obj->epoch,
                                      coh_obj->key.segment_id);
    gsva_tlb_stable_set(page_va, coh_obj->epoch, coh_obj->key.segment_id);

    if (!tlb_hit || ubc_trace_data_path_enabled()) {
        qemu_log("GSVA_TLB: lookup va=%#" PRIx64 " state=%s"
                 " is_write=%u cpu=%" PRIu32 " segment_id=%#" PRIx64
                 " epoch=%" PRIu64 " local_pa=%#" PRIx64 "\n",
                 page_va, gsva_coh_state_name(coh_obj->state),
                                ...
                                uint8_t *resp, uint32_t *resp_len)
{
    ...
    switch (hdr->opcode) {
    case SIM_DEC_OP_GSVA_HOME_V1: {
        const GsvaHomeRequest *request;
        BusControllerDev *ubc = g_sim_decoder && g_sim_decoder->bcs ?
            g_sim_decoder->bcs->ubc_dev : NULL;
        int error;

        G_STATIC_ASSERT(sizeof(GsvaHomeRequest) == 112);
        if (!ubc || len != sizeof(*hdr) + sizeof(*request) ||
            hdr->payload_len != sizeof(*request)) {
        ...
        qemu_log("GSVA_HOME: operation=%u segment=%#" PRIx64
                 ...
                 (uint64_t)request->identity.key.epoch,
                 (uint64_t)request->export_mem_id, error);
        resp_hdr->status = error == GSVA_OK ? SIM_DEC_STATUS_SUCCESS :
            error == GSVA_ERR_COH_PENDING ? SIM_DEC_STATUS_RESOURCE_BUSY :
